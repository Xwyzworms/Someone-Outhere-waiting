...
setwd("D:\\CourseraCourses\\StatisticsWithR\\Inference Statistics\\Week1")
library(statsr)
library(dplyr)
library(shiny)
library(ggplot2)
...
ggplot(data = ames , aes(x = area)) + 
  geom_histogram(binwidth = 300 , col = "blue" , fill = "green" ,aes(y = ..density..)) +
  geom_density(size = 2)
```
...
```{r area-stats}
ames %>% 
  dplyr::summarise(mu = mean(area) , pop_med = median(area),
...
ggplot(data = samp1 , aes(x = area)) + 
  geom_histogram(binwidth = 80,aes(y = ..density..) , fill = "green" , col = 'red')+
  geom_density(size = 2)
# Most Likely Right Skewed ,because we take small sample size ,and it's interval between other is high.

## Let's See The summaries
samp1 %>%
  dplyr::summarise(Smean = mean(area) , Smed = median(area), 
                   Ssd = sd(area) , Siqr = IQR(area) , 
                   Smin = min(area) , Smax = max(area),
                   Squan_1 = quantile(area , 0.75 ))

## Well The mean is approximately nearly same , but notice that this is only one samples , if we take more and more samples , it's approximately same
```
...
```{r sampling-dist}
# type your code for the Exercise here, and Run Document
paste("The Element Total : " ,nrow(sample_means50))

ggplot(data = sample_means50, aes(x = x_bar)) +
  geom_histogram(binwidth = 20) +
  labs(title = mean(sample_means50$x_bar))
...
ames %>%
  ...
  dplyr::summarise(PriceMean = mean(price))
...
```{r price-sampling}
# type your code for this Exercise here, and Run Document
sample_means50 <-ames %>%rep_sample_n( size = 50 , rep = 5000 , replace = T) %>%
  summarise(x_bar = mean(price))

# ggplot(data = sample_means50,aes(x = x_bar)) +
#   geom_histogram(binwidth = 1500 ,aes(y = ..density.. ,fill = "green" , col = "red")) +
#   geom_density(size = 2) 
cat("Mean from its sample ",mean(sample_means50$x_bar) , "\nMean From Population : ",mean(ames$price))


## Normal Sampling , Size and total samples you calculate will make your distrbution approximately normal , and can mirror the population mean 
...
# type your code for this Exercise here, and Run Document
sample_means150 <-ames %>%rep_sample_n( size = 500 , rep = 5000 , replace = F) %>%
  summarise(x_bar = mean(price))

# ggplot(data = sample_means150,aes(x = x_bar)) +
#   geom_histogram(binwidth = 1500 ,aes(y = ..density.. ,fill = "green" , col = "red")) +
#   geom_density(size = 2) 
cat("Mean from its sample ",mean(sample_means150$x_bar) , "\nMean From Population : ",mean(ames$price))

## What i get here is that the more big sample size ,your sampling distribution will closer to normality ,if it's already large enough its will mirrowed the population mean .
```
...
# type your code for this Exercise here, and Run Document
ames %>% sample_n(size = 15 , replace =F )%>%
  dplyr::summarise(x_bar =mean(price))
```
...
# type your code for this Exercise here, and Run Document
sample_means15 <-ames %>%rep_sample_n( size = 50 , rep = 2000, replace = T) %>%
  summarise(x_bar = mean(price))

 ggplot(data = sample_means15,aes(x = x_bar)) +
   geom_histogram(aes(y = ..density.. ,fill = "green" , col = "red")) +
   geom_density(size = 2) 
cat("Mean from its sample ",mean(sample_means15$x_bar) , "\nMean From Population : ",mean(ames$price))
...
sample_means15 <-ames %>%rep_sample_n( size = 150 , rep = 2000, replace = T) %>%
  summarise(x_bar = mean(price))
...
# the more Sample size you have the more normality it's
