...
ggplot2::ggplot(data = samp , aes(x = area))+
  geom_histogram(col = "blue" , aes(y = ..density..))+
  geom_density(size = 2)+
  labs(title = mean(samp$area))

#Right Skewed, mean > median confirmed ,but its  okay since we have 
#n > 30 and indepent , we can apply clt and confidence interval.
lol <- ames %>% rep_sample_n(size = 60 , reps = 10000 ,replace =  T)
var(ames$area)
var(lol$area)
mean(lol$area)


pbinom(q = 2,size = 5 , prob)
?pbinom
# We can make point estimate for mean around 1499.337
```
...
**Answer**
The sample distribution must be nearly normal,with CLT it's already done.
...
**Answer** : 
95 % of random samples of size 60 will yield confidence intervals that contain the true average area of houses in Ames.
...
zstar = qnorm(0.975) ## Z Score ,i.e Critical value 

confindence <- samp %>% summarise(lower = mean(area) - (zstar * sd(area) / sqrt(n)), 
                                  upper = mean(area) + (zstar * sd(area) / sqrt(n)))

paste("With 97.5% Confidence interval and 60 sample size,we expect that true population the average population area will falls between this interval ","(" , round(confindence$lower,2) , "," , round(confindence$upper,2) , ")" , "And the true population mean is : " , round(mean(ames$area),2) , "And Sample Distribution Mean : ",round(mean(lol$area),2) )
...
lol$contain_mu <- ifelse(lol$lower < params$mu & lol$upper > params$mu , "yes" , "no")

lol_data <- data.frame(ci_id = c(1:50 , 1:50) , 
                       ci_bounds = c(ci$lower , ci$upper),
                       captured_mu = c(ci$capture_mu , ci$capture_mu))

ggplot(data = lol_data, aes(x = ci_bounds, y = ci_id, 
                           group = ci_id, color = captured_mu)) +
  geom_point(size = 2) +  # add points at the ends, size = 2
  geom_line() +           # connect with lines
  geom_vline(xintercept = params$mu, color = "darkgray") # draw vertical line

paste("Proportion Contain the confidence Interval : " , 4 / 50)
