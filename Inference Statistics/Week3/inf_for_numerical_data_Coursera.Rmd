...
nc %>%
  group_by(habit) %>%
  summarise(mean_weight = mean(weight) , IQR = IQR(weight) , median = median(weight))
```
...
inference(data = nc, y = weight ,x = habit ,
          statistic = "mean",
...
inference(y = weeks, data = nc, statistic = "mean", type = 'ci', null = 0, 
  alternative = "twosided", method = "theoretical",conf_level = 0.90)
...
```{r gained-mature-ht}
# type your code for the Exercise here, and Knit
inference(data = nc , y = weight , x = mature , 
          type = "ht" , null = 0 , 
          alternative = "twosided" ,
          method = "theoretical" ,statistic = "mean" )
```
...
nc %>% group_by(mature) %>%
        summarise(minage = min(mage) , maxage = max(mage))
