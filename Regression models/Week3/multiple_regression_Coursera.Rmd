...
about the initial scatterplot? well if we looked here , if that's true our data randomly scattered well i guess the correllation here is low.
</div>
```{r jitter}
# type your code for the Exercise here, and Knit
ggplot(data = evals, aes(x = bty_avg, y = score)) +
...
# Nearly normal residuals: Residuals are right skewed, but the sample size is large, so this may not be an important violation of conditions.s
...
plot(m_bty_gen$residuals)# Correctly Interpreted
qqnorm( m_bty_gen$residuals ) # correctly ...
...
model changed the parameter estimate for `bty_avg`? Yep it changed
</div>
```{r summary-mlr-model}
# type your code for the Exercise here, and Knit
...
## Asnwer :  0.12 points higher than minority professors, all else held constant
<div id="exercise">
